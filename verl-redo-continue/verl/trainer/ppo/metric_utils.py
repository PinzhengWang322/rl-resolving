...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    advs = advantages[:,0]
    score = torch.tensor(batch.non_tensor_batch['score'].tolist())
    acc = torch.tensor(batch.non_tensor_batch['acc'])
    redo = torch.tensor(batch.non_tensor_batch['redo'])
    len_penalty = torch.tensor(batch.non_tensor_batch['penalty'])

    metrics = {
        # accuracy
        "accuracy/right": (acc & (~redo)).float().mean().item(),
        "accuracy/wrong": ((~acc) & (~redo)).float().mean().item(),
        "accuracy/redo": redo.float().mean().item(),
        "accuracy/redo_mean": score[redo].float().mean().item(),
        "accuracy/redo_adv": advs[redo].float().mean().item(),
        "accuracy/len_penalty": len_penalty[redo].float().mean().item(),
        # score
