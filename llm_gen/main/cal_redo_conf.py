...
def is_correct_by_math_eval(pred: str | None, gt: str) -> float:
    ...
    try:
        score = math_lst_eval([gt], [pred_str])[0]
...
def needs_redo(resp: str) -> bool:
    ...
    if resp is None:
        return True
    tail = resp[-100:].lower()
    # 包含 "better to redo" 或最后100字符中没有 "boxed" 即视为 redo
...
def mean_confidence_interval(values, alpha: float = 0.05):
    """
    ...
    if n == 1:
        return m, m, m
    s = stdev(values)
    # 简单用 95% 的 z 值；如果 alpha 不是 0.05 也先用这个近似
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="每题最多使用多少条 response（会在洗牌后截断）",
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
        default="\pah\to\your\modelt",
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="需要计算置信区间的采样规模 k（默认 128）",
    )
    parser.add_argument(
        "--ci_alpha",
        type=float,
        default=0.05,
        help="置信区间的 alpha（默认 0.05 -> 95%% CI）",
    )
    ...
    for idx, ex in enumerate(tqdm(ds, desc="Evaluating dataset (full)")):
        ...
        # 洗牌 + 截断；对含 REDO_STR 的做截断处理
        rnd = random.Random(42 + idx)
        rnd.shuffle(responses)
        responses = responses[: args.num]
        responses = [
            (i if REDO_STR not in i else i.split(REDO_STR)[0] + REDO_STR)
            for i in responses
        ]
    ...
    print(
        f"redo所需平均轮数: ",
        (sum(avg_rounds) / len(avg_rounds)) if avg_rounds else 0.0,
    )
    if avg_rounds:
    ...
    # 这里专门为 target_k=128（或者你指定的规模）收集 response 级别分数
    # 只收集“no redo”的 response，且如果某一次采样全是 redo，则这次采样的该题不会贡献样本
    response_scores_for_ci = []
    ...
    if sizes:
        ...
        for k in sizes:
            ...
            for scores_all, redo_all, n_i in zip(
                per_item_scores_all, per_item_redo_all, per_item_N
            ):
                if n_i == 0:
                    continue
                kk = min(k, n_i)
                ...
                for _ in range(args.sample_times):
                    ...
                    # redo 规则准确率
                    no_redo_scores_sub = [
                        s for s, r in zip(s_sub, r_sub) if not r
                    ]
                    if no_redo_scores_sub:
                        acc_k = mean(no_redo_scores_sub)
                    else:
                        acc_k = mean(s_sub)
                    ...
                    # 如果当前规模就是 target_k，则累积到 response 级别样本
                    # 按你的需求：只有 redo 的情况在 CI 中忽略，所以只加入 no_redo_scores_sub
                    if k == args.target_k and no_redo_scores_sub:
                        response_scores_for_ci.extend(no_redo_scores_sub)
            ...
            if per_q_round_means:
                ...
                valid_rounds_k = [
                    x for x in per_q_round_means if x != max_round_k
                ]
                effective_ratio_k = (
                    len(valid_rounds_k) / len(per_q_round_means)
                    if per_q_round_means
                    else 0.0
                )
                effective_mean_rounds_k = (
                    mean(valid_rounds_k) if valid_rounds_k else 0.0
                )
            else:
    ...
    # ===== 规模为 target_k 的 response 级别置信区间 =====
    if response_scores_for_ci:
        m_ci, lo_ci, hi_ci = mean_confidence_interval(
            response_scores_for_ci, alpha=args.ci_alpha
        )
        print(
            f"\n===== 规模为 k={args.target_k} 的 response 级别置信区间（只统计 no-redo 的 response）====="
        )
        print(f"样本数（response 数）：{len(response_scores_for_ci)}")
        print(f"平均得分：{m_ci:.6f}")
        print(
            f"{(1 - args.ci_alpha) * 100:.1f}% 置信区间: +-{(hi_ci - m_ci):.6f}"
        )
    else:
        print(
            f"\n[提示] 在规模 k={args.target_k} 下，没有任何包含 no-redo 的采样，无法计算置信区间。"
        )
